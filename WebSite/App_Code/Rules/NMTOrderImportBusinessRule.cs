...
{
	public partial class NMTOrderImportBusinessRule : MyCompany.Rules.SharedBusinessRules
    {
        ...
        protected override void ExecuteAction(ActionArgs args, ActionResult result)
        {
            ...
            {
                using (SqlProcedure sp = new SqlProcedure("sp_NMT_Order_Execute"))
                {
            ...
            {
                using (SqlProcedure sp = new SqlProcedure("sp_NMT_Order_ClearData"))
                {
                    sp.ExecuteNonQuery();
                    result.Refresh();
                }
