...
{
	public partial class NPTForcastImportBusinessRule : MyCompany.Rules.SharedBusinessRules
    {
        ...
        protected override void ExecuteAction(ActionArgs args, ActionResult result)
        {
            ...
            {
                using (SqlProcedure sp = new SqlProcedure("sp_NPT_Order_Execute"))
                {
            ...
            {
                using (SqlProcedure sp = new SqlProcedure("sp_NPT_Order_ClearData"))
                {
                    sp.ExecuteNonQuery();
                    result.Refresh();
                }
