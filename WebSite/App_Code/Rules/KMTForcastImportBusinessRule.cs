...
{
	public partial class KMTForcastImportBusinessRule : MyCompany.Rules.SharedBusinessRules
    {
        ...
        protected override void ExecuteAction(ActionArgs args, ActionResult result)
        {
            ...
            {
                using (SqlProcedure sp = new SqlProcedure("sp_KMT_Forcast_Execute"))
                {
            ...
            {
                using (SqlProcedure sp = new SqlProcedure("sp_KMT_Forcast_ClearData"))
                {
                    sp.ExecuteNonQuery();
                    result.Refresh();
                }
