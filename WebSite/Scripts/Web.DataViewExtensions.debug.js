...
Web.SmartTag = function () {
    this.initialize();
}

Web.SmartTag.prototype = {
    isVisible: function () {
    ...
    _createTag: function () {
        ...
        var unhoverFunc = function (event) {
            Web.SmartTag._instance.hide(event);
        ...
        var tag = {
            left: $('<div class="web-smart-tag left"></div>').appendTo(document.body).css({ position: 'absolute', width: '1px' }).hide().hover(hoverFunc, unhoverFunc).click(tagClickFunc),
            top: $('<div class="web-smart-tag top"></div>').appendTo(document.body).css({ position: 'absolute', height: '1px' }).hide().hover(hoverFunc, unhoverFunc).click(tagClickFunc),
            right: $('<div class="web-smart-tag right"></div>').appendTo(document.body).css({ position: 'absolute', width: '1px' }).hide().hover(hoverFunc, unhoverFunc).click(tagClickFunc),
            bottom: $('<div class="web-smart-tag bottom"></div>').appendTo(document.body).css({ position: 'absolute', height: '1px' }).hide().hover(hoverFunc, unhoverFunc).click(tagClickFunc),
            arrow: $('<div class="web-smart-tag arrow"><span class="button"></span></div>').appendTo(document.body).css({ position: 'absolute' }).hide().hover(hoverFunc, unhoverFunc).click(tagClickFunc),
            className: null,
    ...
    _createMarker: function () {
        ...
        if (!marker)
            marker = {
                left: $('<div class="web-smart-tag left marker"></div>').appendTo(document.body).css({ position: 'absolute', width: '1px' }).hide().hover(hoverFunc, unhoverFunc),
                top: $('<div class="web-smart-tag top marker"></div>').appendTo(document.body).css({ position: 'absolute', height: '1px' }).hide().hover(hoverFunc, unhoverFunc),
                right: $('<div class="web-smart-tag right marker"></div>').appendTo(document.body).css({ position: 'absolute', width: '1px' }).hide().hover(hoverFunc, unhoverFunc),
                bottom: $('<div class="web-smart-tag bottom marker"></div>').appendTo(document.body).css({ position: 'absolute', height: '1px' }).hide().hover(hoverFunc, unhoverFunc),
                arrow: $('<div class="web-smart-tag arrow marker"><span class="button"></span></div>').appendTo(document.body).css({ position: 'absolute' }).hide().hover(hoverFunc, unhoverFunc)
            };
        return marker;
    },
    unmark: function () {
...
Web.SmartTag._instance = new Web.SmartTag();
...
jQuery.fn.smartTag = function (selector, options /*container, itemEnumerator, test*/) {
    ...
    /// <returns type="jQuery" />
    var args = arguments;
    var instance = Web.SmartTag._instance;
    if (args.length == 2) {
        this.hover(
                function () {
                    var showTag = true;
...
$(document)
    .mousedown(function (event) {
        ...
        if (container.length > 0) {
            ...
            if (dataView) {
                Web.DataView.Extensions.active(dataView.extension(), true);
                activated = true;
            }
        }
        if (!activated)
            Web.DataView.Extensions.active(null, false);
    })
    .keydown(function (event) {
        var extension = Web.DataView.Extensions.activeTarget(event);
        if (extension)
...
Web.DataView.Extensions.activeTarget = function (event) {
    var extension = Web.DataView.Extensions.active();
...
Web.DataView.Extensions.active = function (extension, activate) {
    if (arguments.length == 0) {
        var activeExtensionId = Web.DataView._activeExtensionId;
    ...
    if (activate == null)
        return Web.DataView._activeExtensionId == extension.dataView().get_id();
    else {
        ...
        if (activate) {
            ...
            Web.DataView._activeExtensionId = extension.dataView().get_id();
            extension.activate();
        }
        else {
            Web.DataView._activeExtensionId = null;
            if (activeExtension)
                activeExtension.deactivate();
            if (extension)
...
if (Sys.Browser.agent != Sys.Browser.InternetExplorer || Sys.Browser.version > 7)
    Web.DataView.Extensions.DataSheet = function (dataView) {
        return new Web.DataView.DataSheet(dataView);
    }

Web.DataView.DataSheet = function (dataView) {
    this.dataView(dataView);
}

Web.DataView.DataSheet.prototype = {
    initialize: function () {
    ...
    _createContainer: function () {
        ...
        this.$that().remove();
        var container = $('<div>').attr('id', id).addClass('container data-sheet resizable-data-view').appendTo(element);
    ...
    charWidth: function (columns) {
        if (columns == 0)
            columns = 15;
        var wdvds = Web.DataView.DataSheet;
    ...
    refresh: function (dataRows) {
        ...
        var dvr = Web.DataViewResources.Form;
    ...
    _allocateCells: function () {
        ...
        $('.header-cell', $dataSheet).each(function (index) {
            ...
            if (f.AllowSorting) {
                var sortButton = $('<span class="button"></span>').appendTo(this).text(f.HeaderText).attr('title', String.format(Web.DataViewResources.HeaderFilter.SortBy, f.HeaderText));
        ...
        this._$pageShowing = this.$that('div.pager span.showing').smartTag('pager-menu', {
            ...
            items: function () {
                var items = [];
                $(Web.DataViewResources.Pager.PageSizes).each(function (index) {
                    var pageSize = this;
                    items.push({
                        text: String.format('{0} {1}', Web.DataViewResources.Pager.ItemsPerPage, this),
        ...
        var $sliderPageInfo = this._$sliderPageInfo = $('<div>')
        ...
        var verticalScrollbar = this._$verticalScrollbar = this.$that('_VSB .scroll-bar')
            .slider({
                ...
                change: function (event, ui) {
                    $sliderPageInfo.show().position({ my: 'right-8 middle', at: 'left middle', of: $(this).find('a.ui-slider-handle') });
                },
    ...
    cursor: function (show, row, col) {
        var dataView = this._dataView;
        if (!Web.DataView.Extensions.active(this))
            show = false;
        var $dataSheet = this.$that();
