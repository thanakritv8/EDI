...
public partial class Controls_NPTForcastTemplate : System.Web.UI.UserControl
{
    ...
    protected void AsyncFileUpload1_UploadedComplete(object sender, AjaxControlToolkit.AsyncFileUploadEventArgs e)
    {
        ...
        filename = String.Format("NPT_{0}_FORCAST_{1}", DateTime.Now.ToString("yyyyMMddHHmmss"), filename);
                            ...
                            if (!String.IsNullOrEmpty(data[236].Trim()))
                            {
                                foreach (KeyValuePair<int, string> item in DueDate)
                                {
                                    ...
                                    string DeliveryDate = string.Format("{0}-{1}-{2}"
                                        ...
                                        , String.Format("{0:00}", Convert.ToInt32(_dueDate[1].Substring(8, 4).Trim().Replace("th", "").Replace("st", "").Replace("nd", "").Replace("rd", "").Replace(")", "")))
                                        );

                                    
                                    DateTime _deliveryDate = DateTime.Now;
                                    if (DateTime.TryParse(DeliveryDate, out _deliveryDate))
                                    {
                                        //DeliveryDate + string.Format("{0:00}", montData[1].Substring(7,montData[1].Length-1).Replace("th","").Replace("st","").Replace("nd","").Replace("rd",""))
                                        MyCompany.Data.Objects.NPTForcastImport Order = new MyCompany.Data.Objects.NPTForcastImport();    
